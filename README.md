...
- Naver Maps JavaScript API / MapLibre GL JS PMTiles overlay
...
### 4. Naver Maps 및 PMTiles 설정

지도 배경은 Naver Maps JavaScript API를 계속 사용합니다. PMTiles는 네이버 지도 위의 투명 MapLibre overlay 경계 레이어로만 사용합니다. Vercel 환경 변수에는 `NEXT_PUBLIC_NAVER_MAP_CLIENT_ID`를 유지하고, Naver Cloud Platform의 인증 도메인에 로컬/배포 URL을 등록해야 합니다.
...
- Vercel에 `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY` 등록
...
현재 메인 지도는 Naver Maps 배경 + MapLibre PMTiles overlay 구조입니다. `components/NaverMap.tsx` 파일명은 기존 참조 호환을 위해 유지합니다.

- 배경지도: Naver Maps JavaScript API
- 경계 레이어: `public/tiles/eupmyeondong.pmtiles`
- PMTiles source id: `eupmyeondong`
- PMTiles source-layer: `eupmyeondong`
- `NEXT_PUBLIC_NAVER_MAP_CLIENT_ID`는 네이버 배경지도 렌더링에 필요합니다.