...
전국 읍면동 단위로 방문 기록, 일기, 사진을 남기는 개인 여행 지도 일기장입니다. 네이버 지도를 배경으로 사용하고, 전국 읍면동 경계는 PMTiles 벡터 타일을 MapLibre GL JS 투명 overlay로 렌더링합니다.

## 주요 기능

- 전국 읍면동 경계 표시
- 방문 횟수 단계별 파란색 그라데이션 시각화
- 지도별 방문 기록, 일기, 사진 관리
- 지도 생성, 선택, 수정, 공유, 삭제
- Supabase Auth 기반 로그인/회원가입/프로필
- 선택 지역별 타임라인과 전체 기록 타임라인
- 사용자가 직접 고르는 기록 날짜(`entry_date`)
- 타임라인 정렬: 기록 날짜 최신순, 기록 날짜 오래된순, 작성일 최신순
- 방문한 지역 수, 총 방문 횟수, 가장 많이 간 지역, 방문 비율 통계
- 방문 전, 1회, 2~3회, 4~6회, 7회 이상 방문 상태 범례
...
- 색상 규칙: 방문 전은 옅은 회하늘색, 방문 횟수가 많아질수록 `#BDE8F5` → `#4988C4` → `#1C4D8D` → `#0F2854` 순서로 진해집니다.
...
- 통계: 방문한 지역 수, 총 방문 횟수, 가장 많이 간 지역, 방문 비율
- 현황: 방문 전, 1회 방문, 2~3회 방문, 4~6회 방문, 7회 이상 방문, 통계 상위 지역, 선택된 지역, 범례
- 전체 기록: 현재 지도 전체 지역 기록, `entry_date` 기준 정렬
- 계정: 사용자 정보, 프로필, 로그아웃
